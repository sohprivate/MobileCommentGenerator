...
class CommentGenerationState:
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'target_datetime': self.target_datetime.isoformat() if isinstance(self.target_datetime, datetime) else str(self.target_datetime),
        ...
        return {
            ...
            'generation_metadata': {
                **self.generation_metadata,
                ...
                'weather_condition': getattr(self.weather_data, 'weather_description', None) if self.weather_data else None,
                'temperature': getattr(self.weather_data, 'temperature', None) if self.weather_data else None,
                'selected_past_comments': self._format_selected_comments(),
                'validation_passed': getattr(self.validation_result, 'is_valid', None) if self.validation_result else None,
    ...
    def _format_selected_comments(self) -> List[Dict[str, Any]]:
        ...
        if self.selected_pair:
            if hasattr(self.selected_pair, 'weather_comment') and self.selected_pair.weather_comment:
