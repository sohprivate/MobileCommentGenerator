...
def mock_input_node(state: CommentGenerationState) -> CommentGenerationState:
    ...
    ユーザー入力を受け取り、初期状態を設定
    """
    logger.info(f"InputNode: 地点={state.get('location_name')}, 時刻={state.get('target_datetime')}")
...
def mock_output_node(state: CommentGenerationState) -> CommentGenerationState:
    ...
    logger.info(f"生成完了: {state.get('final_comment')} (実行時間: {state.get('execution_time_ms')}ms)")
