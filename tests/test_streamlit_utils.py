...
class TestLocationUtils:
    ...
    def test_load_locations(self, mock_location_manager):
        ...
        mock_manager_instance.get_all_location_names.return_value = [
            "東京", "大阪", "稚内", "那覇"
        ]
...
class TestHistoryManagement:
    ...
    @patch('builtins.open', new_callable=mock_open, read_data='[{"location": "東京", "comment": "晴れです"}]')
    @patch('os.path.exists')
    ...
    def test_save_history_preserves_limit(self, mock_exists, mock_file):
        ...
        # 既存の履歴（100件）
        existing_history = [
            {"timestamp": f"2024-06-{i:02d}", "comment": f"コメント{i}"}
