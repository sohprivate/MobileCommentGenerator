...
システムはデフォルトで**12時間後の天気予報**を使用してコメントを生成します。この設定は環境変数で変更可能です。
...
設定した時刻は、UIの各地域詳細情報で「🕐 予報時刻」として表示されます。表示される時刻は日本標準時（JST）に自動変換されます。

**注意**: この設定により、すべてのコンポーネントで指定した時刻の天気データが統一的に使用されます。
