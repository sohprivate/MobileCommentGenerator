...
**雨天時の制限:**
- 生活関連の不適切なアドバイスを除外（「洗濯物を外に」「布団を干す」「外干しを」など）
- 雨天時に矛盾する表現を防止（「中休み」「晴れ間」「回復」「乾燥」など）
- 大雨時は「折り畳み傘」「軽い雨具」「短時間の外出」など軽い対策も制限
- 雷検出時の重要度を降水量に基づいて動的に調整

**温度ベース制限の詳細化:**
- 中程度の暖かさ（25-33°C）: 「厳しい暑さ」「酷暑」「危険な暑さ」を制限
- 32°C未満では「熱中症」警告を抑制し、より適切な表現に誘導
- 地域特性の考慮: 沖縄では雪・極寒表現を除外、北海道では極暑表現を除外

**その他の検証強化:**
- 湿度80%以上で「乾燥」表現を禁止
- 曇天時は「スッキリしない」のみ許可し、晴天表現を制限
- 降水量と表現の一貫性チェック（降水0mmで雨表現を禁止）

**文字列類似度計算の最適化:**
- 短文（10文字以下）のみを対象として計算コストを削減
- 長さが2倍以上異なる場合は早期に類似度が低いと判定
- 文字集合の重複計算によるO(n)の効率的な処理

**システム改善:**
- ログレベル調整: 一部のcriticalレベルをdebug/infoに変更（段階的に実施中）
